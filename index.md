...
range of publicly available historical and forecast meteorology data
from agencies across the world. The free access tier allows for 10,000
API calls per day. The paid tiers increase the number of daily API
calls. Learn more about the Open-Meteo project at their website
...
on Earth using
[`get_forecast()`](http://flare-forecast.org/ropenmeteo/reference/get_forecast.md)
(max 16 days, all variables returned in m/s, °C, mm, etc.):
...
(max 35 days, 30–51 ensemble members depending on model):
...
See the
[vignette](http://flare-forecast.org/ropenmeteo/vignettes/example_usage.Rmd)
for full examples covering ensemble forecasts, historical weather,
seasonal forecasts, climate projections, GLM integration, and the EFI
standard.
